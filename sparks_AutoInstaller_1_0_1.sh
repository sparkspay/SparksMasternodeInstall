  ...
  if [ -e $CONFIGFOLDER/$CONFIG_FILE ]; then
    ...
    spk_version=${spk_version%,*}
        ...
        if [[ $spk_version -lt $COIN_VERSION ]]; then
          ...
          echo -e ""
          echo -e "Would you like to ${GREEN}upgrade[U] ${RED}$COIN_NAME ${NC} or preform a ${GREEN}fresh install[f]${NC} [Y/n] : "
          echo -e ""
          echo -e "An upgrade will keep the current blockchan, sentinel and $COIN_NAME configuiration "
          echo -e "Ubuntu and only the ${RED}$COIN_NAME ${NC} deamon and CLI will be upgraded"
          echo -e ""
